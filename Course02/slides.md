...
title: |
  Session 2\
  Feature Engineering, Missing Data & Classical Models
...
what it throws away, and it is often the larger term — that is the whole picture.
Point at the two coloured pieces: the left one you would have had anyway, the right
one exists only because values were missing.
:::
...
::: notes
The hinge of the session. The first half was about the columns: how to build them,
and what to do about the holes. Everything from here on assumes that table exists and
asks the next question — before we spend the rest of the semester on neural networks,
we look under the hood of the models that still win on tabular data. Not a catalogue
— a handful of mechanisms, each one drawn.
The second half is deliberately light on text: the text is me.
:::
## Classical models
...
That is also the running order of the second half, so this slide is its table of
contents.
...
Every model in the second half fills the box differently, but nothing outside the box
...
opposite trade from every other model we have seen, and it is why k-NN is awkward in
...
::: notes
The summary slide of the session: the same 300 points, every model we built, in the
